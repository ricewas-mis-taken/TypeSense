# ⚠️⚠️ IF YOU ARE ON VERSION 1.1.0 or earlier, YOU WILL NOT AUTO-UPDATE TO THE NEWEST VERSION, NOR WILL YOUR DATA BE SENT TO THE SERVER. DOWNLOAD THE LATEST VERSION FROM RELEASES⚠️⚠️
...
Participants can suppress survey popups from the tray icon with two independent toggles: **Do Not Disturb** (pauses surveys until toggled back off or the app restarts) and **Gaming Mode** (same suppression, but also tags logged windows `gaming=True`). Logging continues under both. Suspect/gamed windows are also filtered out automatically during dataset compilation (`data_comp.py`) before training.

A survey only fires when the participant was genuinely active: at least 4 of the ~80 fifteen-second windows in the 20-minute interval must clear the per-window keystroke floor, and the most recent keystroke must be within 5 minutes of the survey firing (otherwise they're likely AFK).
...
| `gaming` | Whether "Gaming Mode" was active for this window | Lets training exclude gaming-session typing patterns |
...
keylogger.py                           client_typesenseML.py (Flask)
  ...
  ├── tray icon (ID / survey / DND / Gaming)  └── surveys/[session].csv
