###WARNING: RELEASE VERSION v1.0.1 AND ALL PAST VERSIONS ARE BUGGED; PLEASE DOWNLOAD THE LATEST ONE!!!
...
* **Minimum keystroke count per window** — windows with too few keystrokes are dropped as low-confidence, since sparse windows produce noisy statistics
...
keylogger.py                           client_typesenseML.py (Flask)
  ...
  └── user_send.py                           └── surveys/[session].csv
        ├── send()        ──HTTPS──►
        ├── send_survey() ──HTTPS──►
        ├── offline queue (local)
        └── retry loop (30s)
